...
import com.dxzo.bukkit.InfusedEffects.ArmorEquipEvent.EquipMethod;
...
public class ArmorListener implements Listener{
	...
	public final void inventoryClick(final InventoryClickEvent e){
		...
		if(getInventoryClicked(e) != null && !getInventoryClicked(e).getType().equals(InventoryType.PLAYER)) return;
		...
		if(shift){
			...
			if(numberkey){
				if(getInventoryClicked(e).getType().equals(InventoryType.PLAYER)){// Prevents shit in the 2by2 crafting
					// e.getClickedInventory() == The players inventory
					// e.getHotBarButton() == key people are pressing to equip or unequip the item to or from.
					// e.getRawSlot() == The slot the item is going to.
					// e.getSlot() == Armor slot, can't use e.getRawSlot() as that gives a hotbar slot ;-;
					ItemStack hotbarItem = getInventoryClicked(e).getItem(e.getHotbarButton());
					if(!isAirOrNull(hotbarItem)){// Equipping
						newArmorType = ArmorType.matchType(hotbarItem);
						newArmorPiece = hotbarItem;
						oldArmorPiece = getInventoryClicked(e).getItem(e.getSlot());
					}else{// Unequipping
	...
	// 		if(!isAirOrNull(i)){
	// 			Bukkit.getServer().getPluginManager().callEvent(new ArmorEquipEvent(p, EquipmentSlot.DEATH, ArmorType.matchType(i), i, null));
    ...
    private Inventory getInventoryClicked(InventoryClickEvent event) {
